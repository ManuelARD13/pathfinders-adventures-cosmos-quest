/*Available Classes Images Src By Raze and Genre*/
const assasinImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/igWDLOj.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/EEt6eC3.png",
        femaleProfile: ""
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/USIQtEb.png",
        female: new Image().src = "https://i.imgur.com/9Sqe8ME.png",
        maleProfile:  new Image().src = "https://i.imgur.com/mtT3tL1.png",
        femaleProfile: new Image().src = "https://i.imgur.com/vLsS5Un.png"
    },
    orc: {
        male: new Image().src="https://i.imgur.com/7tiOEWc.png",
        female: new Image().src = "https://i.imgur.com/uzaD2L5.png",
        maleProfile:  new Image().src = "https://i.imgur.com/jhy9ZA8.png",
        femaleProfile: new Image().src = "https://i.imgur.com/ppihHzc.png"
    },
    dwarf: {
        male: "https://i.imgur.com/yQDyyGN.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/iidKH7B.png",
        femaleProfile: new Image().src = ""
    }
}

const barbarianImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/nUDphU4.png",
        female: new Image().src = "",
        maleProfile:  new Image().src = "https://i.imgur.com/10PiNio.png",
        femaleProfile: new Image().src = ""
    },
    elf: {
        male: new Image().src = "",
        female: new Image().src = ""
    },
    orc: {
        male: new Image().src = "https://i.imgur.com/T1eeF1q.png",
        female: new Image().src = "https://i.imgur.com/9njgtd0.png",
        maleProfile:  new Image().src = "https://i.imgur.com/Z8OG4rp.png",
        femaleProfile: new Image().src = "https://i.imgur.com/fTiyQbj.png"
    },
    dwarf: {
        male: new Image().src = "https://i.imgur.com/uAQfgpo.png",
        female: new Image().src = "https://i.imgur.com/5bVjNDB.png",
        maleProfile:  new Image().src = "https://i.imgur.com/1wtWFNw.png",
        femaleProfile: new Image().src = "https://i.imgur.com/ouIhlgK.png"
    }
}

const berzerkerImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/6WbdhJx.png",
        female: new Image().src = "https://i.imgur.com/NwcU00v.png",
        maleProfile:  new Image().src = "https://i.imgur.com/0Ez42FN.png",
        femaleProfile: new Image().src = "https://i.imgur.com/FNtCocj.png"
    },
    elf: {
        male: "",
        female: ""
    },
    orc: {
        male: new Image().src = "https://i.imgur.com/skBm0fj.png",
        maleProfile:  new Image().src = "https://i.imgur.com/wSlBa7g.png",
    },
    dwarf: {
        male: "",
        female: ""
    }
}

const casterImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/aEAWOy2.png",
        female: new Image().src = "https://i.imgur.com/32iUoVl.png",
        maleProfile:  new Image().src = "https://i.imgur.com/BZFXMZs.png",
        femaleProfile: new Image().src = "https://i.imgur.com/HID1Ytq.png"
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/8Vp5na6.png",
        female: new Image().src = "https://i.imgur.com/z6daOLY.png",
        maleProfile:  new Image().src = "https://i.imgur.com/q7HXCmR.png",
        femaleProfile: new Image().src = "https://i.imgur.com/SH5R1uA.png"
    },
    orc: {
        male: "",
        female: ""
    },
    dwarf: {
        male: new Image().src = "https://i.imgur.com/bgaudC8.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/fTTjbNU.png",
        femaleProfile: ""
    }
}

const celestialDefenderImg = {
    elf: {
        male: new Image().src = "https://i.imgur.com/4lYJq5N.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/1zut60p.png",
        femaleProfile: ""
    }
}

const crusaderImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/3iZExzD.png",
        female: new Image().src =  "https://i.imgur.com/23UlmAL.png",
        maleProfile:  new Image().src = "https://i.imgur.com/RvC7Ro8.png",
        femaleProfile: new Image().src = "https://i.imgur.com/WRcFZVb.png"
    }
}

const dragonTaimerImg = {
    elf: {
        male: "",
        female: new Image().src = "https://i.imgur.com/bt11zNT.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/edBdVwz.png"
    }
}

const explorerImg = {
    dwarf: {
        male: new Image().src = "https://i.imgur.com/ROdxAjq.png",
        female: new Image().src = "https://i.imgur.com/ABvHxed.png",
        maleProfile:  new Image().src = "https://i.imgur.com/EJM5PYI.png",
        femaleProfile: new Image().src = "https://i.imgur.com/8JFpvzr.png"
    }
}

const fighterImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/OHpp7Oo.png",
        female: new Image().src = "https://i.imgur.com/FgALwiY.png",
        maleProfile:  new Image().src = "https://i.imgur.com/fhztMIw.png",
        femaleProfile: new Image().src = "https://i.imgur.com/wQ2up3R.png"
    },
    elf: {
        male: "",
        female: ""
    },
    orc: {
        male: "",
        female: new Image().src = "https://i.imgur.com/c39qO0y.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/4SjSQsr.png"
    },
    dwarf: {
        male: "",
        female: ""
    }
}

const hunterImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/lPGuCPn.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/y8td23P.png",
        femaleProfile: new Image().src = ""
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/dxjGWOD.png",
        female: new Image().src = "https://i.imgur.com/IO9Ml0s.png",
        maleProfile:  new Image().src = "https://i.imgur.com/WKgjrth.png",
        femaleProfile: new Image().src = "https://i.imgur.com/FKeaZOa.png"
    },
    orc: {
        male: new Image().src = "",
        female: new Image().src = "https://i.imgur.com/l2cLuvF.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/lLu1L9a.png"
    },
    dwarf: {
        male: new Image().src = "",
        female: "",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = ""
    }
}

const ironLordImg = {
    dwarf: {
        male: new Image().src = "https://i.imgur.com/qCMYqd8.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/W6cIewp.png",
        femaleProfile: new Image().src = ""
    }
}

const knightImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/2cxk7Ln.png",
        female: new Image().src = "https://i.imgur.com/xavsi52.png",
        maleProfile:  new Image().src = "https://i.imgur.com/spVXqgd.png",
        femaleProfile: new Image().src = "https://i.imgur.com/n2Nao1W.png"
    },
    elf: {
        male: "",
        female: new Image().src = "https://i.imgur.com/y4MBR12.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/JFgaURz.png"
    }
}

const necromancerImg = {
    orc: {
        male: new Image().src = "https://i.imgur.com/YiX1bSK.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/Ir8JRZo.png",
        femaleProfile: new Image().src = ""
    }
}

const paladinImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/ghHCjd5.png",
        female: new Image().src = "",
        maleProfile:  new Image().src = "https://i.imgur.com/nCOP7W0.png",
        femaleProfile: new Image().src = ""
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/VrxPw6E.png",
        female: new Image().src = "https://i.imgur.com/0aUfDoo.png",
        maleProfile:  new Image().src = "https://i.imgur.com/Xh4ti11.png",
        femaleProfile: new Image().src = "https://i.imgur.com/TY1MCwJ.png"
    },
    orc: {
        male: "",
        female: ""
    },
    dwarf: {
        male: new Image().src = "https://i.imgur.com/ery07m6.png",
        female: new Image().src = "https://i.imgur.com/ghxnEFr.png",
        maleProfile:  new Image().src = "https://i.imgur.com/t20PNkl.png",
        femaleProfile: new Image().src = "https://i.imgur.com/4Lw7LUl.png"
    }
}

const priestImg = {
    human: {
        male: "",
        female: new Image().src = "https://i.imgur.com/49zMDpm.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/ZmemOOR.png"
    },
    elf: {
        male: "",
        female: new Image().src = "https://i.imgur.com/juUKHdF.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/QKZgzYo.png"
    },
    orc: {
        male: "",
        female: new Image().src = "https://i.imgur.com/t5WSRon.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/UdLDYzY.png"
    },
    dwarf: {
        male: new Image().src = "https://i.imgur.com/7JYAj6C.png",
        female: new Image().src ="https://i.imgur.com/X4iN0Mn.png",
        maleProfile:  new Image().src = "https://i.imgur.com/S7XeQux.png",
        femaleProfile: new Image().src = "https://i.imgur.com/Z3qdMc9.png"
    }
}

const rangerImg = {
    human: {
        male: "",
        female: new Image().src = "https://i.imgur.com/TZ2caiQ.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/zPzmzyY.png"
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/UKpJDDD.png",
        female: new Image().src = "https://i.imgur.com/o6zCwsW.png",
        maleProfile:  new Image().src = "https://i.imgur.com/HN1ORYS.png",
        femaleProfile: new Image().src = "https://i.imgur.com/feYZLxM.png"
    },
    orc: {
        male: new Image().src = "https://i.imgur.com/Q3sG9xF.png",
        female: new Image().src = "https://i.imgur.com/PngUT3v.png",
        maleProfile:  new Image().src = "https://i.imgur.com/TeKxdqK.png",
        femaleProfile: new Image().src = "https://i.imgur.com/5bltIx7.png"
    },
    dwarf: {
        male: new Image().src = "https://i.imgur.com/jl1JXe4.png",
        female: new Image().src = "https://i.imgur.com/DT63NQS.png",
        maleProfile:  new Image().src = "https://i.imgur.com/kquBLdj.png",
        femaleProfile: new Image().src = "https://i.imgur.com/hTHZTp1.png"
    }
}

const riderImg = {
    human: {
        male: new Image().src ="https://i.imgur.com/UqbSsnk.png",
        female:new Image().src =  "https://i.imgur.com/XQKliij.png",
        maleProfile:  new Image().src = "https://i.imgur.com/10eYgoZ.png",
        femaleProfile: new Image().src = "https://i.imgur.com/hiApm2X.png"
    },
    orc: {
        male: new Image().src ="https://i.imgur.com/hAJ4VDY.png",
        maleProfile:  new Image().src = "https://i.imgur.com/1mh8xaq.png",
    }
}

const shamanImg = {
    human: {
        male: "",
        female: new Image().src = "https://i.imgur.com/pYzAkKU.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/3QCzjWJ.png"
    },
    orc: {
        male: new Image().src = "https://i.imgur.com/vhl2xjJ.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/u5TNhu5.png",
        femaleProfile: new Image().src = ""
    },
    dwarf: {
        male: new Image().src = "https://i.imgur.com/ftr2oMa.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/EOWSfze.png",
        femaleProfile: new Image().src = ""
    }
}

const samuraiImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/FZ9qrlM.png",
        female: new Image().src = "https://i.imgur.com/GfXfHTv.png",
        maleProfile:  new Image().src = "https://i.imgur.com/myQvyFc.png",
        femaleProfile: new Image().src = "https://i.imgur.com/cZEYFA2.png"
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/nTkb883.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/Zcp2gb0.png",
        femaleProfile: new Image().src = ""
    }
} 

const warchiefImg = {
    orc: {
        male: new Image().src = "https://i.imgur.com/jHBA2I4.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/CfSMjsf.png",
        femaleProfile: new Image().src = ""
    }
}

const warlockImg = {
    human: {
        male: "",
        female: new Image().src = "https://i.imgur.com/uasLdpu.png",
        maleProfile:  new Image().src = "",
        femaleProfile: new Image().src = "https://i.imgur.com/58lWcIL.png"
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/tjnOHmO.png",
        female: new Image().src = "https://i.imgur.com/OHwUOfr.png",
        maleProfile:  new Image().src = "https://i.imgur.com/hYtMwWU.png",
        femaleProfile: new Image().src = "https://i.imgur.com/JTwb4te.png"
    },
    orc: {
        male: new Image().src = "https://i.imgur.com/fzkwQo2.png",
        female: new Image().src = "https://i.imgur.com/Z5qEmAt.png",
        maleProfile:  new Image().src = "https://i.imgur.com/iXFQZ5h.png",
        femaleProfile: new Image().src = "https://i.imgur.com/9DkHViJ.png"
    },
    dwarf: {
        male: new Image().src = "https://i.imgur.com/e3PbGvu.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/cJtnLHT.png",
        femaleProfile: new Image().src = ""
    }
}

const warriorImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/5YGdKHn.png",
        female: new Image().src = "https://i.imgur.com/NRfSBw9.png",
        maleProfile:  new Image().src = "https://i.imgur.com/1t2kFPD.png",
        femaleProfile: new Image().src = "https://i.imgur.com/SNLsajI.png"
    },
    elf: {
        male: new Image().src = "https://i.imgur.com/zPFPUxv.png",
        female: new Image().src ="https://i.imgur.com/FoSH4XU.png",
        maleProfile:  new Image().src = "https://i.imgur.com/weddYvt.png ",
        femaleProfile: new Image().src = "https://i.imgur.com/tWRIHcp.png"
    },
    orc: {
        male: new Image().src = "https://i.imgur.com/sWlqn1t.png",
        female: new Image().src ="https://i.imgur.com/WCg7MMH.png",
        maleProfile:  new Image().src = "https://i.imgur.com/UCG817N.png",
        femaleProfile: new Image().src = "https://i.imgur.com/QxF5BhY.png"
    },
    dwarf: {
        male: new Image().src ="https://i.imgur.com/syipRkn.png",
        female: "",
        maleProfile:  new Image().src = "https://i.imgur.com/irkIddl.png",
        femaleProfile: new Image().src = ""
    }
}

/*
Potencial New Classes

const bardImg = {
    human: {
        male: new Image().src = "https://i.imgur.com/kfBCU27.png",
        female: new Image().src = "https://i.imgur.com/rETBX2v.png"
    },
    orc: {
        male: new Image().src = "https://i.imgur.com/dex8chU.png",
    }
}

const druidImg = {
    human: {
        male: ,
        female: new Image().src = "https://i.imgur.com/ltJW69l.png" 
    }
    elf: {
        male: new Image().src = "https://i.imgur.com/ibHlYex.png",
        female: 
    }
}

const gunslingerImg = {
    human: {
        male: "",
        female: new Image().src = https://i.imgur.com/gUCrieh.png
    }
}
*/

/*Constructors*/
const createNewRaze = (razeName, razeSkills ,razeLore, razeBKImg, razeImgFemale, razeImgMale, availableClassesObj, dualRaze=false, razeModifiers = {CON: 1, STR: 1, DEX: 1, INT: 1, WIS:1, CHA: 1}) => ({
    key: razeName,
    razeName: razeName,
    razeSkills: razeSkills,
    razeLore: razeLore,
    razeBKImg: razeBKImg,
    dualRaze: dualRaze,
    razeImgFemale: razeImgFemale,
    razeImgMale: razeImgMale,
    availableClasses: availableClassesObj,
    razeModifiers: razeModifiers,
})

const createNewClass = (className, classSkills, classLore, classIcon ,imgObj ,classRequirements, hitPoints=12, dualClass=false) => ({
    className: className,
    classSkills: classSkills,
    classLore: classLore,
    classIcon: classIcon,
    classImg: imgObj,
    classRequirements: classRequirements,
    hitPoints: hitPoints,
    dualClass: dualClass,
    
})

/*Available Classes*/
const playableClasses = []

//Future Funcionality
//Dual Classes

// const dragonSlayer = createNewClass("dragonSlayer", ["Dragon Killer", "Scales Skin", "Fire's Breath"], "lorem impsum x 100")
//playableClasses.push(dragonSlayer)

/*Standar Classes */
const assasin = createNewClass(
        "Assasin", 
        ["Stealh Mastery", "Fatal Blow", "Anatomic Acknowledge", "Small Weapons Proficiency"], 
        
        "A mercenary undertaking his task with cold, professional detachment, the assassin is equally adept at espionage, bounty hunting, and terrorism. At his core, an assassin is an artisan, and his medium is death. Trained in a variety of killing techniques, assassins are among the most feared classes.", 
        
        "https://i.imgur.com/eKEOTZ3.png", 
        assasinImg,
        {DEX: 14, INT: 10},
        8
    )
playableClasses.push(assasin)

const barbarian = createNewClass(
        "Barbarian", 
        ["Untamable Rage","Beast Reflexes", "Taunt", "Feral Intimidation"], 
        
        "Barbarians excel in combat, possessing the martial prowess and fortitude to take on foes seemingly far superior to themselves. With rage granting them boldness and daring beyond that of most other warriors, barbarians charge furiously into battle and ruin all who would stand in their way.",
        
        "https://i.imgur.com/vVWdxTB.png",
        barbarianImg,
        {CON: 15, STR: 12},
        12
     )
playableClasses.push(barbarian)

const berzerker = createNewClass(
        "Berzerker", 
        ["Berzerk Fury", "Monstrous Resistence", "Blood Hunger", "Brain Crushed"], 
        
        "A Berserker relishes in the glory of combat. Their hedonistic pleasure from battle wounds is borderline masochistic. While enraged, they enter a sort of higher state where pain is just another form of indulgence. Extremely resistents, overwhelmingly strong and with a untameble rage; Berzerkers are a doble-edge ally or the most dangerous of foes.", 
        
        "https://i.imgur.com/FDuaBg8.png", 
        berzerkerImg, 
        {CON: 16, STR: 15},
        20
    )
playableClasses.push(berzerker)

const caster = createNewClass(
        "Caster", 
        ["Magical Nature", "Arcane Bond", "Arcane Literature", "Arcane School"], 
        
        "While some seek to control their abilities through meditation and discipline, becoming masters of their fantastic birthright, others give in to their magic, letting it rule their lives with often explosive results. Regardless, casters live and breathe that which other spellcasters devote their lives to mastering, and for them magic is more than a boon or a field of study; it is life itself. casters excel at casting a selection of favored spells frequently, making them powerful battle mages. As they become familiar with a specific and ever-widening set of spells, casters often discover new and versatile ways of making use of magics other spellcasters might overlook. Their bloodlines also grant them additional abilities, assuring that no two casters are ever quite alike.", 
        
        "https://i.imgur.com/jBBTATN.png", 
        casterImg,
        {INT: 14, CHA: 10},
        6
    )
playableClasses.push(caster)

const celestialDefender = createNewClass(
    "Celestial Defender", 
    ["Sword of Damocles", "Heavens Shield", "Lay on Hands", "Vow of Justice"], 
    
    "Celestial Defenders are the fiercest of heavenly warriors, making up the most and best of the Blessed Army, train in heaven whenever an elf rise as a Gods champion. Empowered with divine might, they are perfect bastions of light guided by righteous wrath and an otherwise good disposition, making them feared by evildoers and respected by less vile creatures. Most usually acting as soldiers, knights, or paladins for a good cause, they revel in the adrenaline of battle, and seek evil wherever it may hide.", 
    
    "https://i.imgur.com/fVjIEzp.png", 
    celestialDefenderImg,
    {CON: 16, CHA: 16},
    20
    )
playableClasses.push(celestialDefender)

const crusader = createNewClass(
    "Crusader", 
    ["Holy Quest", "Divine Aura", "Evil Smite", "Vow of Faith"], 

    "Crusaders serve the militant arm of a church, ready to stand guard over the religion's holy places and to be its swift, avenging arm against those who resist its truth.", 
    
    "https://i.imgur.com/VChKkyj.png", 
    crusaderImg, {CON: 14, WIS: 10},
    )
playableClasses.push(crusader)

const dragonTamer = createNewClass(
    "Dragon Tamer", 
    ["Dragon Breathing", "Draconic Senses", "Elemental Domain", "Dragon Raider"],

    "Rise by and with dragons and wyverns, Dragon Tamers are the perfect bridge between this magestic creatures and the humanoids around them. They swear to protect the dragon's lands and childrens with their lifes and the tremendous power they have inherited from their ancient bond with them. Beware but the Dragon Tamers do not recognize Good or Evil, they just live, prey and kill for whatever their dragons teach to them.", 

    "https://i.imgur.com/8f6Z4dy.png", 
    dragonTaimerImg, 
    {STR: 16, CHA: 16},
    )
playableClasses.push(dragonTamer)

const fighter = createNewClass(
        "Fighter", 
        ["Weapon Mastery", "Battle Genius", "Flurry of Blows", "Mindfull Breath"], 
        
        "Lords of the battlefield, fighters are a disparate lot, training with many weapons or just one, perfecting the uses of armor, learning the fighting techniques of exotic masters, and studying the art of combat, all to shape themselves into living weapons. Far more than mere thugs, these skilled warriors reveal the true deadliness of their weapons, turning hunks of metal into arms capable of taming kingdoms, slaughtering monsters, and rousing the hearts of armies. Soldiers, knights, hunters, and artists of war, fighters are unparalleled champions, and woe to those who dare stand against them.", 
        
        "https://i.imgur.com/wtpA7oK.png",
        fighterImg,
        {CON: 12, DEX: 16},
        8,
     )
playableClasses.push(fighter)

const explorer = createNewClass(
    "Explorer", 
    ["Treasure Hunter", "Mystery's Scroll", "Street Wisdom", "Universal Knowledge"], 
    
    "Brave invetigators and scavengers of lost or forgotten knowledge, Explorers are quintessential adventurers, exploring the world for esoteric truths, magical and mundane relics and artifacts, and new and mysterious vistas, be they wonderful or terrible. For some, this journey is purely about the wealth, for others the glory of discovery, and still more are stirred by the irresistible drive to uncover the bones of the ages and legends of forgotten epochs in an effort to chronicle the deeds of yesterday, today, and tomorrow.", 
    
    "https://i.imgur.com/F7jpnCj.png", 
    explorerImg, 
    {INT: 16, WIS: 14}, 
    8
)
playableClasses.push(explorer)

const hunter = createNewClass(
    "Hunter", 
    ["Favored Foe", "Headshoot", "Hunting Mastery", "Animal Instinct"], 
        
    "Be they scouts, trackers, or bounty hunters, rangers share much in common: unique mastery of specialized weapons, skill at stalking even the most elusive game, and the expertise to defeat a wide range of quarries. Knowledgeable, patient, and skilled hunters, these rangers hound man, beast, and monster alike, gaining insight into the way of the predator, skill in varied environments, and ever more lethal martial prowess.", 
    
    "https://i.imgur.com/kz6Thjo.png", 
    hunterImg, 
    {DEX: 14, WIS: 10},
    8
)
playableClasses.push(hunter)

const ironLord = createNewClass(
    "Iron Lord", 
    ["Father of Dwarfs", "Lawbringer Hammer", "Runic Knowledge", "Ragnarok"], 

    "Father of all. Ruler of the nine realms and God of War. Obsesivly lurking for magical powers beyond imagination, The Iron Lord is the most powerfull of all dwarfs, their maximun king. An expert on runic lenguage, earth resources, weapon enchantment and military leadership, The Iron Lord only live for Conquest of the most powerfull magic sources of the world. ", 

    "https://i.imgur.com/GXlwQDg.png", 
    ironLordImg, 
    {STR: 16, WIS: 16},
    20
)
playableClasses.push(ironLord)

const knight = createNewClass(
    "Knight", 
    ["Resplendant Morale", "Vow of Honor", "Signature Weapon", "Duel Master"], 
    
    "The knight is a warrior aristocrat, skilled in war and steeped in social grace. Most come from powerful noble families, while others are minor nobles or even commoners who evince some skill at arms. The iconic knight's arms are the lance, sword, and hand weapons, but other knights may favor the katana and bow or the lance and spatha. A knight is distinguished from the fighter by his embrace of a code of honor that embodies the traditions of his society, order, or school. Emboldened and exalted by these principles, the knight exists as an inspiration to others, a match of valor and elegance who rises above his frailties to achieve great glory.", 
    
    "https://i.imgur.com/aEurYCh.png", 
    knightImg, 
    {STR: 12, DEX: 16},
    10
)
playableClasses.push(knight)

const necromancer = createNewClass(
    "Necromancer", 
    ["March of the Death", "Death Touch", "Channel Energy", "Deadly Sanctuary"], 
    
    "The goddess of death gives power over her dominion as freely as Life brings hers into the world. Wizards and clerics, in particular, master the art of necromancy through the lens of their studies and their faith. But what of those who draw their power directly from the goddess? Who do not merely worship her, but allow her will and her power to suffuse their very beings? These are the true necromancers, the dread followers of Death who carefully walk the line between the mortal concept of living and dead. They see beyond both into the churning heart of reality with the power to breed life and snuff it out in the blink of an eye.", 
    
    "https://i.imgur.com/PBDRvxw.png", 
    necromancerImg, 
    {INT: 12, CHA: 16},
    8
)
playableClasses.push(necromancer)

const paladin = createNewClass(
        "Paladin", 
        ["Holy Aura", "Evil Vanquisher", "Divine Hands", "Holy Favor"], 
        
        "Knights, crusaders, and lawbringers, paladins seek not just to spread divine justice but to embody the teachings of the virtuous deities they serve. In pursuit of their lofty goals, they adhere to ironclad laws of morality and discipline. As reward for their righteousness, these holy champions are blessed with boons to aid them in their quests: powers to banish evil, heal the innocent, and inspire the faithful. Although their convictions might lead them into conflict with the very souls they would save, paladins weather endless challenges of faith and dark temptations, risking their lives to do right and fighting to bring about a brighter future.",

        "https://i.imgur.com/3hL4EQl.png",
        paladinImg,
        {CON: 12, CHA: 14},
        10
     )
playableClasses.push(paladin)

const priest = createNewClass(
    "Priest",
     ["Divine Caster", "Divine Aura", "Deity's Artifact", "Channel Energy"],

     "In faith and the miracles of the divine, many find a greater purpose. Called to serve powers beyond most mortal understanding, all priests preach wonders and provide for the spiritual needs of their people. Clerics are more than mere priests, though; these emissaries of the divine work the will of their deities through strength and the magic of their gods. Devoted to the tenets of the religions and philosophies that inspire them, these ecclesiastics quest to spread the knowledge and inf luence of their faith",

    "https://i.imgur.com/3R5ShQH.png",
    priestImg,
    {WIS: 12, CHA: 14},
    8,
 )
playableClasses.push(priest)

const ranger = createNewClass(
        "Ranger",
        ["Animal Companion", "Breath of the Wild", "Animal Instincs", "Feral Empathy"], 
    
        "Hunters are warriors of the wilds that have forged close bonds with trusted animal companions. They focus their tactics on fighting alongside their companion animals as a formidable team of two. Able to cast a wide variety of nature spells and take on the abilities and attributes of beasts, hunters magically improve both themselves and their animal companions.",  
        
        "https://i.imgur.com/KvJoPwZ.png", 
        rangerImg,
        {DEX: 12, CHA: 15},
        10
    )
playableClasses.push(ranger)

const rider = createNewClass(
    "Rider", 
    ["Exceptional Mount", "Charge Battle", "Long Weapons Mastery", "Tamer Spirit"], 
    
    "The cavalier is defined not only by his dedication to his order or his skill on the battlefield, but also by the special relationship he maintains with his mount. Where some cavaliers are simply skilled with horses or well-trained knights, the beast rider spends his life in constant pursuit of the most perfect mount, forming bonds with greater, more powerful, and more exotic creatures.", 
    
    "https://i.imgur.com/dK3YbuB.png",
    riderImg, 
    {DEX: 12, CHA: 14},
    10
    )
playableClasses.push(rider)

const shaman = createNewClass(
    "Shaman", 
    ["Spirit Animal", "Totemic Control", "Lightning Dance", "Mystic Epiphany"], 
    
    "While some heroes speak to gods or consort with otherworldly muses, shamans commune with the spirits of the world and the energies that exist in every living thing. These divine adventurers draw upon their power to shape the world and expand the influence of their spiritual patrons. Shamans have strong ties to natural spirits. They form powerful bonds with particular spirits, and as their power grows they learn to call upon other spirits in times of need.", 
    
    "https://i.imgur.com/2fBE6dH.png", 
    shamanImg, 
    {CON: 12, WIS: 14},
    8 )
playableClasses.push(shaman)

const samurai = createNewClass(
    "Samurai", 
    ["Samurai Challenge", "Vow of Honor", "Unbreakable Discipline", "Xing-Long's Sword"], 
    
    "Few warriors are more dedicated to honor and the code of the warrior than the samurai. Trained from an early age in the art of war and sworn to the service of a lord, the samurai holds a position of power and respect in many lands, often serving as the voice and justice of the local noble. The samurai takes on his training with zeal, learning the way of the blade (typically a katana), the bow, and the horse. Some even learn how to effectively use firearms, if they are available. The samurai is often the most trusted soldier in his lord’s employ. In him, the common folk see honor and sacrifice. He is an honorable warrior, dedicated to the realm and the leaders that guide it.", 
    
    "https://i.imgur.com/M8C6sIW.png", 
    samuraiImg, 
    {STR: 16, DEX: 16},
    10
    )
playableClasses.push(samurai)

const warchief = createNewClass(
    "Warchief", 
    ["Horde Warcry", "Fury of the Elements", "Beasts Leadership", "Doomhammer"], 
    
    "Warchief of the Horde. Son of Durotan, liberator of the orcs and chieftain of the Frostwolf clan. As the greatest shaman ever known, he stepped down to join the Earthen Ring in stabilizing the world following the Cataclysm.", 
    
    "https://i.imgur.com/jposVjE.png", 
    warchiefImg, 
    {STR: 16, CHA: 16},
    20
    )
playableClasses.push(warchief)

const warlock = createNewClass(
    "Warlock", 
    ["Arcane Bloodline", "Demonic Dealing", "Magical Nature", "Alchemyst"],
    
    "Scions of innately magical bloodlines, the chosen of deities, the spawn of monsters, pawns of fate and destiny, or simply flukes of fickle magic, Warlocks look within themselves for arcane prowess and draw forth might few mortals can imagine. Emboldened by lives ever threatening to be consumed by their innate powers, these magic-touched souls endlessly indulge in and refine their mysterious abilities, gradually learning how to harness their birthright and coax forth ever greater arcane feats. Just as varied as these innately powerful spellcasters abilities and inspirations are the ways in which they choose to utilize their gifts. While some seek to control their abilities through meditation and discipline, becoming masters of their fantastic birthright, others give in to their magic, letting it rule their lives with often explosive results. Regardless, Warlocks live and breathe that which other spellcasters devote their lives to mastering, and for them magic is more than a boon or a field of study; it is life itself.", 
    
    "https://i.imgur.com/KgFdPED.png", 
    warlockImg, 
    {INT: 10, CHA: 14},
    6
)
playableClasses.push(warlock)

const warrior = createNewClass(
    "Warrior", 
    ["Weapon Mastery", "Power Warcry", "Tactil Battle", "Campaign Wrestler"], 
    
    "Warriors are melee fighters highly trained in the arts of weaponry. These mighty warriors, strong and quick on the battlefield, depend on their heavy plate armor, shields, and battle prowess to defend themselves and ensure enemies don't focus on their softer-skinned allies. Weapon mastery is a staple of warrior training, and they are capable of delivering high damage with enormous two-handed weapons or by wielding two weapons. As warriors deal or take damage, their rage grows, allowing them to deliver truly crushing attacks in the heat of battle.", 
    
    "https://i.imgur.com/cNSh8f1.png",
    warriorImg, 
    {STR: 14, DEX: 10},
    12
)
playableClasses.push(warrior)

/*Available Razes*/
const playableRazes = []

const humanAvailableClasses = {
    /*List of Classes to display after select this especific Raze*/
    male: [
        assasin,
        barbarian,
        berzerker,
        /*bard,*/
        caster,
        crusader,
        fighter,
        hunter,
        knight,
        paladin,
        rider,
        samurai,
        warrior
    ],
    female: [
        /*barbarian,*/
        /*bard,*/
        berzerker,
        caster,
        crusader,
        /*druid, */
        fighter,
        /*gunslinger, */
        knight,
        priest,
        ranger,
        rider,
        samurai,
        shaman,
        warrior,
        warlock
    ]
}

const human = createNewRaze(
    "human", 
    ["Weapon Proficiency", "General's Leadership", "Talented Apprentice", "Adventurer"], 

    "Humans possess exceptional drive and a great capacity to endure and expand, and as such are currently the dominant race in the world. Their empires and nations are vast, sprawling things, and the citizens of these societies carve names for themselves with the strength of their sword arms and the power of their spells. Humanity is best characterized by its tumultuousness and diversity, and human cultures run the gamut from savage but honorable tribes to decadent, devil-worshiping noble families in the most cosmopolitan cities. Human curiosity and ambition often triumph over their predilection for a sedentary lifestyle, and many leave their homes to explore the innumerable forgotten corners of the world or lead mighty armies to conquer their neighbors, simply because they can", 

    new Image().src="https://i.imgur.com/Htwa9J6.jpg", 
    "https://i.imgur.com/GfgzOBw.png", 
    "https://i.imgur.com/fgt8E0D.png", 
    humanAvailableClasses, 
    true,
    {CON: 0, STR: 0, DEX: 0, INT: 2, WIS: -1, CHA: 2}
)
playableRazes.push(human)

const elfAvailableClasses = {
    /*List of Classes to display after select this especific Raze*/
    male: [
        assasin,
        /*bard,*/
        caster,
        celestialDefender,
        /*druid,*/
        hunter,
        paladin,
        ranger,
        samurai,
        warrior,
        warlock
    ],
    female: [
        assasin,
        caster,
        dragonTamer,
        /*druid, */
        hunter,
        knight,
        paladin,
        priest,
        ranger,
        /*samurai,*/
        warrior,
        warlock
    ]
}
const elf = createNewRaze(
        "elf", 
        ["Mystical Perception", "Blood Linage Wisdom", "Elven Inmunity & Magic", "Low-light Vision"], 

        "The long-lived elves are children of the natural world, similar in many superficial ways to fey creatures, yet different as well. Elves value their privacy and traditions, and while they are often slow to make friends, at both the personal and national levels, once an outsider is accepted as a comrade, such alliances can last for generations. Elves have a curious attachment to their surroundings, perhaps as a result of their incredibly long lifespans or some deeper, more mystical reason. Elves who dwell in a region for long find themselves physically adapting to match their surroundings, most noticeably taking on coloration reflecting the local environment.",

        new Image().src="https://i.imgur.com/SylK7Kb.png",
        "https://i.imgur.com/ldlwtYl.png",
        "https://i.imgur.com/jY083KY.png",
        elfAvailableClasses,
        true,
        {CON: -2, STR: 0, DEX: +2, INT: +2, WIS: 0, CHA: 0}
    )
playableRazes.push(elf)

const orcAvailableClasses = {
    /*List of Classes to display after select this especific Raze*/
    male: [
        assasin,
        barbarian,
        berzerker,
        /*bard,*/
        /*hunter,*/
        necromancer,
        ranger,
        rider,
        shaman,
        warchief,
        warrior,
        warlock
    ],
    female: [
        assasin,
        barbarian,
        fighter,
        hunter,
        priest,
        ranger,
        warrior,
        warlock
    ]
}

const orc = createNewRaze(
        "orc", 
        ["Brutal Intimidation", "Beast's Authority", "Berzerk Ferocity", "Dark Vision"], 

        "Orcs are aggressive, callous, and domineering. Bullies by nature, they respect strength and power as the highest virtues. On an almost instinctive level, orcs believe they are entitled to anything they want unless someone stronger can stop them from seizing it. They rarely exert themselves off the battlefield except when forced to do so; this attitude stems not just from laziness but also from an ingrained belief that work should trickle down through the pecking order until it falls upon the shoulders of the weak. They take slaves from other races, orc men brutalize orc women, and both abuse children and elders, on the grounds that anyone too feeble to fight back deserves little more than a life of suffering. Surrounded at all times by bitter enemies, orcs cultivate an attitude of indifference to pain, vicious tempers, and a fierce willingness to commit unspeakable acts of vengeance against anyone who dares to defy them.", 

        "https://i.imgur.com/WOHtwQE.jpg", 
        "https://i.imgur.com/5iPKo3V.png", 
        "https://i.imgur.com/BHSVxDH.png", 
        orcAvailableClasses, 
        true, 
        {CON: 2, STR: 4, DEX: 0, INT: -2, WIS: -2, CHA: -2}
    )
playableRazes.push(orc)

const dwarfAvailableClasses = {
    /*List of Classes to display after select this especific Raze*/
    male: [
        assasin,
        barbarian,
        caster,
        explorer,
        ironLord,
        paladin,
        priest,
        ranger,
        shaman,
        warrior,
        warlock
    ],
    female: [
        explorer,
        paladin,
        priest,
        ranger,
        warrior,
    ]
}

const dwarf = createNewRaze(
        "dwarf", 
        ["Dark Vision", "Stonecunning", "Slowly and Steady", "Hardy"],

        "Dwarves are a stoic but stern race, ensconced in cities carved from the hearts of mountains and fiercely determined to repel the depredations of savage races like orcs and goblins. More than any other race, the dwarves have acquired a reputation as dour and humorless craftsmen of the earth. It could be said that dwarven history shapes the dark disposition of many dwarves, for they reside in high mountains and dangerous realms below the earth, constantly at war with giants, goblins, and other such horrors.",
 
        new Image().src="https://i.imgur.com/ssNrVRu.jpg", 
        "https://i.imgur.com/ZyqGfTo.png", 
        "https://i.imgur.com/pHbKy9T.png", 
        dwarfAvailableClasses, 
        true, 
        {CON: 2, STR: 0, DEX: 0, INT: 0, WIS: 2, CHA: -2}
    )
playableRazes.push(dwarf)

export { playableClasses, playableRazes }